...
while running:
    ...
    for generation in range(num_generations):
        while(not all(robot.crash for robot in robots) or not all(robot.finish for robot in robots)):
            for robot in robots:
                robot.move()
                robot.vxg = 20 * (robots.index(robot) + 1)
                robot.theta_d = -(robot.sensor_data[len(environment.edge_distances)-1] - robot.sensor_data[1])*0.01* (robots.index(robot)+1)
            ...
            if robot.finish == True or robot.crash == True:
                robots.remove(robot)
